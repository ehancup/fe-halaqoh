...
import { AddSantriPayload, EditSantriPayload } from "@/lib/santri/interface";
...
const Page = () => {
  ...
  const { useEditSantri, useDetailSantri } = useSantriModule();
  ...
  const { data: detail, isLoading: detailLoad } = useDetailSantri(id);
  ...
  const { mutate, isPending } = useEditSantri(id);
  const formik = useFormik<EditSantriPayload>({
    initialValues: {
      nama_santri: detail?.data.nama_santri || "",
      kelas: detail?.data.kelas || null,
      musrif_id: detail?.data.musrif ? detail.data.musrif.id : null
    },
    validationSchema: addSantriSchema,
    enableReinitialize: true,
    onSubmit: (value: EditSantriPayload) => {
      console.log(value);
      mutate(value, {
